...
k-setup:
	minikube -p dev.to start --cpus 2 --memory=4098; \
        # ingress eh o proxy reverso que vai fazer o load balance pra disponibilizar pro mundo externo 
	minikube -p dev.to addons enable ingress; \
        # graficos
	minikube -p dev.to addons enable metrics-server; \
        # separacao dentro do cluster
	kubectl create namespace dev-to
        # pod eh a menor unidade dentro do kubernates, uma boa pratica eh manter apenas um container por pod
